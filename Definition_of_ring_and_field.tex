...
\begin{dfn*}
	空でない集合$A$に, 二つの演算$+$と$\times$（和と積）が定義されていて,
	次の性質を満たすとき, $A$を\textbf{環}という.
	\begin{enumerate}[label=(\arabic*)\;]
		\item $A$が$+$に関してアーベル群になる.
		\item すべての$a,b \in A$に対して
		$ (ab)c=a(bc) $. ← 結合法則
		\item すべての$a,b \in A$に対して$ (a+b)c=ac+bc $. ← 分配法則
		\item 乗法に対しての単位元$1$がある（全ての$a\in A$に対して$1a=a1=a$）
	\end{enumerate}
\end{dfn*}\leavevmode

$a,b \in A$が$ab=ba$なら, $a,b$は\textbf{可換}.
$A$の任意の元$a,b$が可換なら, $A$は\textbf{可換環}.

$a\in A$に対して、$ab=ba=1$となる元があれば, 
$b$は$a$の逆元であり, $b=a^{-1}$と書く.
$a^{-1}$が存在する場合, $a$は\textbf{可逆元}または\textbf{単元}.
\newline

$ A$の単元全体の集合を$A^\times$と書く.

$ A^\times$は$A$の乗法に関して群になる→$A$の\textbf{乗法群}.

$ A=\{0\},\; 0+0=0,\; 0\cdot 0=0 $と定義すると、$A$は環である.

この環を\textbf{零環}, あるいは\textbf{自明な環}という.
\newline

\begin{tcolorbox}
\begin{prp*}
  $A$を環とするとき, 次の(1)(2)が成り立つ.
...
\begin{proof}
  ...
  (2) 乗法に対しての単位元$1$に関して, 任意の$a \in A$が$a=1a=0a=0$となるので, 
  $A$の元は$0$のみであり, したがって$A$は自明な環である.
...
$ \mathbb{Z} $の元のうち, 逆元が存在するのは$-1$と$1$のみなので, 
$\mathbb{Z}^\times = \{-1,1\} $.
...
\begin{dfn*}
	空でない集合$K$に対して和と積が定義されており,
	次の条件を満たすとき, $K$を\textbf{可除環}と呼ぶ.
	\begin{enumerate}[label=(\arabic*)\;]
		\item 加法と乗法により$K$が環となる.
		\item $1\neq0$、つまり$K$が零環ではない.
		\item 任意の$K\ni a\neq 0$が乗法に関して可逆元である
...
\begin{tcolorbox}
	\begin{prp*}
		...
		$0$から$n-1$までの整数$x,y$により、$\overline{x},\overline{y}$
		という形をした$\mathbb{Z}/n\mathbb{Z}$の二つの元に対し,
		...
		も$xy$を使って同様に定義する. 
        このように定義した演算により, 
		$\mathbb{Z}/n\mathbb{Z}$は可換環となる. 
	\end{prp*}
\end{tcolorbox}

\begin{proof}
	明らかに$\overline{0},\overline{1}$はそれぞれ加法と乗法についての単位元である.
	$\overline{x}\neq \overline{0}$なら,
	$\overline{n-x}+\overline{x}=\overline{0}$であり,
	$\overline{n-x}$は$\overline{x}$の逆元である.

	$\overline{x},\overline{y},\overline{z}\in \mathbb{Z}/n\mathbb{Z}$で,
	$x+y$を$n$で割った余りを$r_1$とするとき,
	$x+y=nq_1+r_1$となる整数$q_1$が存在し, 
	...
	$\overline{x}+(\overline{y}+\overline{z})=\overline{r_3}$
	であることもわかるが, $(x+y)+z=x+(y+z)$なので, $r_2=r_3$である.
