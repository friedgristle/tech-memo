...
\begin{dfn*}
	$G$を群、$x \in G$とする. もし, $x^n=1_G$となる正の整数$n$が存在すれば,
	その中で最小のものを$x$の位数という.
...
\end{tcolorbox}
\begin{proof}
  $x^0 = 1_G$なので, 加法群$\mathbb{Z}$の単位元$0 \subset H $である.
  $ m, n \in H$について, $ x^{m+n} = x^m\cdot x^n = 1_G\cdot 1_G =1_G $
  であるから, $m+n \in H $.
  また$m$の逆元$-m$について,
  $x^{-m} = 1_G\cdot x^{-m} = x^m\cdot x^{-m} = x^{m-m} = 1_G $なので
  $-m \in H$となる.
...
\begin{proof}
  ...
  $1$も割り切れなければならず, 定義と矛盾する.したがって,素数は無限にある.
\end{proof}
...
\begin{tcolorbox}
	\begin{thm*}
	  $a>b>0$を整数とする. $a$を$b$で割った商を$q$, 余りを$r$とする.
...
\end{tcolorbox}
\begin{proof}
  $d$は$a$, $b$の最大公約数なので$ax+by=d$となる$x$, $y$が存在し,
...
\begin{tcolorbox}
	\begin{prp*}
		...
		= \{\overline{m} \mid 0 \leq m < n, \text{mとnは互いに素} \}$
	\end{prp*}
\end{tcolorbox}
\begin{proof}
  $n=1$なら$\mathbb{Z}/n\mathbb{Z}$は零環であり,
...
（$m=0$と$n=1$は互いに素）
  ...
  $\bar{m}\bar{r}=\bar{1}$となる整数$0<r<n$があり
  （$\because$乗法群なので逆元が存在する）,
...
\begin{tcolorbox}
	\begin{thm*}
		$p$が素数なら, $\mathbb{Z}/p\mathbb{Z}$は体である.
	\end{thm*}
\end{tcolorbox}
...
\begin{tcolorbox}
  \begin{prp*}
    $H$が$\mathbb{Z}$の部分群なら, $H=d\mathbb{Z}$となる整数$d\geq0$があり,
	$H=d\mathbb{Z}$である.
  \end{prp*}
\end{tcolorbox}
\begin{proof}
  $H=\{0\}$の場合：$d=0$ととると, $d\mathbb{Z}=0\mathbb{Z}=\{0\}=H$
  となり条件を満たす. したがって以降は,$H\neq \{0\}$と仮定する.
  $H$が零でないならば, 正の元を持つ. つまり$H \cap \mathbb{Z}_{>0}$は
  空ではないので, その中の最小の正整数を$d$と取る. ここで$d>0$である.
  
  $d\in H$かつ$H$は部分群なので任意の整数$k$に対して$kd \in H$.
  よって$d\mathbb{Z} \subseteq  H$.
  任意に$h \in H$を取ると, 整数$q$, $r$が存在して
  $h = qd + r, \quad 0\leq r < d$と表せ,$ qd \in dZ \subseteq H$だから
  $r = h - qd \in H$（部分群だから差が元に残る）.
  ここで$r$は$0 \leq r < d$の整数であるが, $d$は$H$の正の最小元だったので,
  もし$r>0$ならば$r \geq d$でなければならず, 矛盾する. したがって$r=0$である.
  つまり$h=qd$となるから, 任意の$h \in H$は$d\mathbb{Z}$に属し,
  $H \subseteq d\mathbb{Z}$.
...
\begin{tcolorbox}
	\begin{prp*}
		$G$を群, $x \in G$とし, $x$の位数は有限で$d < \infty$とする.
		このとき、$n \in \mathbb{Z}$に対し次の(1)、(2)は同値である.
		さらに,$\{ m \in \mathbb{Z} \mid x^m = 1_G \} = d\mathbb{Z}$である.
...
\begin{proof}
	(2)$\implies$(1)について：\\
	$x$の位数は$d$なので$x^d$=$1_G$. また$n$は整数$q$により$n=qd$と表せる.
	したがって$x^n = x^{qd} = (x^d)^q = 1_G^q = 1_G$.\\
	(1)$\implies$(2)について：\\
	$H=\{n \in \mathbb{Z} \mid x^n = 1_G\}$とおく.\\
	明らかに$0 \in H$である.
	$a,b \in H$なら, $x^{a+b} = x^a x^b = 1_G 1_G = 1_G$なので,
	$a+b \in H$.\\
	また, $x^{-a}=(x^a)^{-1}=1_G^{-1}=1_G$より, $a^{-1}=-a \in H$.\\
	...
	一方, $x^d = 1_G$から$d \in H$であり$H=k\mathbb{Z}$,
	したがって$k \mid d$となり$k \leq d$.\\
	以上より$k=d$, すなわち
	$H=\{ n \in \mathbb{Z} \mid x^n = 1_G \} = d\mathbb{Z}$となるので,
	$d \mid n $（$n$は$d$の倍数）.
\end{proof}\leavevmode

\begin{tcolorbox}
	\begin{prp*}
		$x$を群$G$の位数$d<\infty$の元,
		$H=\langle x \rangle $を$x$で生成された巡回部分群とする.
		このとき, $ |H|=d $である.
	\end{prp*}
\end{tcolorbox}
\begin{proof}
	$H = \{ x^n \mid n \in \mathbb{Z} \} = \{x, x^2, \cdots, x^{d-1}, x^d=1_G \}$である.
	...
	したがって,任意の$ \{i,j \in \mathbb{Z} \mid 0 < i < j \leq d \}$
	に対して$ x^i \neq x^j$である.
\end{proof}\leavevmode

\subsubsection*{演習問題}

\paragraph{2.4.1} (1) 36と-48の最大公約数と最小公倍数を求めよ.
(2) 35と24は互いに素か？\\
...
\paragraph{2.4.2} (1) 395と265の最大公約数$d$をユークリッドの互除法を使って求めよ.

(2) $395x+265y=d$となる整数$x,y$の組を一つ見つけよ.\\

$395 = 265+130,\; 265=2\times 130+5,\; 130=26\times 5$より, $d=5$.
$ 5=265-2\times 130=265 - 2\times(395-265)=-2\times 395 +3\times 265$,
したがって$x=-2,y=3$。\\

\paragraph{2.4.3} (1)$\mathbb{Z}/7\mathbb{Z}$において,
$\bar{2},\cdots,\bar{6}$の乗法に関する逆元を求めよ.

(2) $\mathbb{Z}/284\mathbb{Z}$において, $\bar{3}$の乗法に関する逆元を求めよ.\\

$\bar{2}^{-1}=\bar{4},\; \bar{3}^{-1}=\bar{5},\;\bar{6}^{-1}=\bar{6}$
$284+1=285=3\times 95$で, 95は284と互いに素である. 
よって,$\bar{3}^{-1}=\bar{95}$.\\

\paragraph{2.4.4} $p$が素数で$n>0$が整数なら,
$|(\mathbb{Z}/p^n\mathbb{Z})^\times |=(p-1)p^{n-1}$であることを証明せよ.
\begin{proof}
	$\mathbb{Z}/p^n\mathbb{Z}$の元の個数は$p^n$個.
	このうち$p^n$と互いに素である元のみが$(\mathbb{Z}/p^n\mathbb{Z})^\times$
	の元となる.
...
\paragraph{2.4.5} $G$を群,$x \in G$を位数60の元とするとき,
$x^{35}$の位数を求めよ.\\
$\mathrm{LCM}(60,35)=420$より,
$x^{420}=(x^{60})^7=1_G^7=1_G=(x^{35})^{12}$である.
したがって$x^{35}$の位数は12.\newline

\paragraph{2.4.6} $G$を群, $x \in G$を位数$d<\infty$の元とする.
$n$を整数とするとき, $x^n$の位数を求めよ.\\

$n=0$のとき, $x^n=x^0=1_G$なので, 位数は1.\\
$n\neq0$のとき, $d$と$n$の最小公倍数$\mathrm{LCM}(d,n)$に対して,
$\frac{\mathrm{LCM}(d,n)}{n}=\frac{d}{\mathrm{GCD}(d,n)}$が
$x^n$の位数となる.\newline

\paragraph{2.4.7} $G$が次の群であるとき,$G$を生成する元をすべて求めよ.
...
\paragraph{2.4.8}  群$G$のすべての元$g$が$g^2=1$となるなら, 
$G$はアーベル群であることを証明せよ.
\begin{proof}
  すべての元$g$が$g^2=1$であることから, $a,b \in G$についても$a^2=1,b^2=1$である.
...
\paragraph{2.4.9}  $G=\mathrm{GL}_2(\mathbb{R})$とし,
...
\begin{proof}
	...
	\]
	となるため, これが単位行列$I_2$となる$n$は存在せず, $gh$の位数は無限である.
\end{proof}
↑位数が有限である元の積の位数が有限であるとは限らない.\newline

\paragraph{2.4.10} $G$をアーベル群とする.
...
\begin{proof}
	(1) $a,b \in G$の位数は有限なので, $a^m=1,b^n=1$となる
	$m,n \in \mathbb{Z}$が存在する.
	...
	また(1)より, 位数が有限である$a,b \in H$に対して$ab$の位数も有限であるため,
	$ab \in H$である. $a$の位数が$k$であるとすると,$a$の逆元$a^{-1}\in G$について,
