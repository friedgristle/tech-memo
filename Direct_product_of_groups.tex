...
\begin{proof}
	...
	と定義する.
	$x,y\in k+mn\mathbb{Z}$なら, $x\equiv y$であり,
	$mn \mid (x-y)$となる. このとき$m \mid (x-y),\: n\mid (x-y)$なので
	$y+m\mathbb{Z}=x+m\mathbb{Z},\; y+n\mathbb{Z}=x+n\mathbb{Z}$となる.
	したがって, 写像$\phi$はwell-definedである.

	$a,b\in \mathbb{Z}$に対して
	\begin{align*}
		\phi((a+mn\mathbb{Z})(b+mn\mathbb{Z}))&=\phi(ab+mn\mathbb{Z})\\
		&=(ab+m\mathbb{Z},ab+n\mathbb{Z})\\
